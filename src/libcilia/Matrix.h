...
template<class T = double> class Vector3 ;
...
public:
    ...
    template<class F> void base_apply(F f, const Matrix_base& mat) { for (Index i = 0; i<size(); ++i) f(elem[i], mat.data()[i]); }
    template<class F> void base_apply(F f, const Vector3<T>& mat) { for (Index i = 0; i<size(); ++i) f(elem[i], mat.data()[i]); }
...
public:
    ...
    //vStrange!!!
    Matrix(const Row<T,1>& a) : Matrix_base<T>(a.dim1(),a.elem), d1(a.dim1()) 
    { 
        // std::cerr << "construct 1D Matrix from Row\n";
       // cout << "mover " << endl;
    }

    Matrix(const Matrix&& a) : Matrix_base<T>(a.dim1(),a.elem), d1(a.dim1()) 
    { 
        // std::cerr << "construct 1D Matrix from Row\n";
       // cout << "mover " << endl;
    }
    ...
    Matrix& operator=(const Vector3<T>& a)
        // copy assignment: let the base do the copy
    {
        // std::cerr << "copy assignment (" << this->size() << ',' << a.size()<< ")\n";
        if (d1!=3) error("length error in 1D=");
        //this->base_assign(static_cast<Matrix<T,1>*> &a);
        //copy elements?!
        const T * data = a.data();
        this->elem[0] = data[0];
        this->elem[1] = data[1];
    ...
    //Matrix& operator+=(Matrix& a) { this->base_apply(Add_assign<T>(), a);   return *this; }
    Matrix& operator+=(const Matrix& a) { this->base_apply(Add_assign<T>(), a);   return *this; }
    Matrix& operator+=(const Vector3<T>& a) { this->base_apply(Add_assign<T>(), a);   return *this; }
    //Matrix& operator+=(const Row<T,1>& a) { this->base_apply(Add_assign<T>(), a);   return *this; }
    Matrix& operator-=(const Matrix& a) { this->base_apply(Minus_assign<T>(), a);   return *this; }
    Matrix& operator-=(const Vector3<T>& a) { this->base_apply(Minus_assign<T>(), a);   return *this; }
    ...
    */
    ...
    T sqdistance(const Matrix<T,1>& d2) const {
        T sqdist = 0.0;
        for (int i=0;i<this->size();++i){
            T diff = this->elem[i] - d2[i];
            sqdist+= diff*diff;
            }
        return sqdist;
    }

    void add_mul(const Matrix<T,1>& a, T c) {
    	if (a.size() < this->size()){ cout << a.size() <<" < "<< this->size()<<endl;fflush(stdout);error("sizes wrong for scale_and_add");}
	    for (Index i = 0; i<this->size(); ++i) this->elem[i] += a[i]*c;
    }
    void add_mul3(const Matrix<T,1>& a, T c) {
    	if (a.size() != this->size()-1) error("sizes wrong for scale_and_add");
	    for (Index i = 0; i<this->size()-1; ++i) this->elem[i] += a[i]*c;
    }
};
...
public:
    ...
    void range_check(Index n1, Index n2) const
    {
        ...
        if (n1<0 || d1<=n1) {cout << "to big:" <<n1; error("2D range error: dimension 1");}
    ...
    */
    ...
    Index get_closest_idx(const Matrix<T,1>& point) {
        double min_dist = 9999;//std::numeric_limits<double>::max();
        Index min_idx = 0;
        for (Index i = 0; i<d1; ++i) {
            double dist = (*this)[i].sqdistance(point);
            if (min_dist > dist) {
                min_dist = dist;
                min_idx = i;
            }
...
public:
    ...
    //SR: 1D Methods for particle interaction and bounds 
          Row<T,1> operator()(Index n1, Index n2)       { range_check(n1,n2,0); return Row<T,1>(d3,&this->elem[d2*d3*n1+d3*n2]); }; 
    const Row<T,1> operator()(Index n1, Index n2) const { range_check(n1,n2,0); return Row<T,1>(d3,&this->elem[d2*d3*n1+d3*n2]); };

          Row<T,1> get_row_by_idx(Index n1)       { /*range_check(n1,n2,0);*/ return Row<T,1>(d3,&this->elem[d3*n1]); }; //n1 is correct!
    const Row<T,1> get_row_by_idx(Index n1) const { /*range_check(n1,n2,0);*/ return Row<T,1>(d3,&this->elem[d3*n1]); }; //n1 is correct!
    //-SR
    // slicing (return a row):
...
public:
    ...
    Matrix(const T (&a)[n1][n2][n3][n4]) : Matrix_base<T>(n1*n2*n3), d1(n1), d2(n2), d3(n3), d4(n4)
        // deduce "n1", "n2", "n3" (and "T"), Matrix_base allocates T[n1*n2*n3*n4]
    {
        // std::cerr << "matrix ctor\n";
        for (Index i = 0; i<n1; ++i)
            for (Index j = 0; j<n2; ++j)
                for (Index k = 0; k<n3; ++k)
                    ...
                    this->elem[i*n2*n3+j*n3+k*n4+l]=a[i][j][k][l];
    }
    ...
    {
        // std::cerr << "copy assignment (" << this->size() << ',' << a.size()<< ")\n";
        if (d1!=a.d1 || d2!=a.d2 || d3!=a.d3 || d3!=a.d4) error("length error in 4D =");
        this->base_assign(a);
        return *this;
    }
    ...
    Index dim2() const { return d2; }    // number of elements in a column
    Index dim3() const { return d3; }    // number of elements in a depth
    Index dim4() const { return d4; }    // number of elements in a depth
...
template<class T> T dot_product(const Matrix<T,3>&a , const Matrix<T,3>& b)
{
    if (a.size() != b.size()) error("sizes wrong for dot product");
    T sum = 0;
    const T * data_a =  a.data();
    const T * data_b =  b.data();
    for (Index i = 0; i<a.size(); ++i) sum += data_a[i]*data_b[i];
    return sum;
}
...
public:
    ...
    Matrix<T,1>& operator=(const Vector3<T>& a)
    {
        return *static_cast<Matrix<T,1>*>(this)=a;
    }
};
...
template<class T, int D> Matrix<T,D> operator*(const T& c, const Matrix<T,D>& m) { Matrix<T,D> r(m); return
            r*=c; }
...
template<class T, int D> Matrix<T,D> operator+(const Matrix<T,D>& m, const Matrix<T,D>& m2) { Matrix<T,D> r(m); r.base_apply(Add_assign<T>(),m2);   return r;}
template<class T, int D> Matrix<T,D> operator-(const Matrix<T,D>& m, const Matrix<T,D>& m2) { Matrix<T,D> r(m); r.base_apply(Minus_assign<T>(),m2);   return r;}

template<class T, int D1, int D2> int get_index(Matrix<T,D1>& ptr, Matrix<T,D2>& base) { 
    //cout << ptr.data() << " " << base.data() <<endl;
    return (ptr.data() - base.data())/3;}
